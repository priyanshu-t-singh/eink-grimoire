...
// SanitizeEPUBHTML fixes protocol-relative resource URLs generated by Kavita
func SanitizeEPUBHTML(chapterHTML string, baseURL string) string {
	...
	// 1. If base URL without scheme appears (e.g. `//192.168.0.1:5000/`), prefix with scheme
	...
	if strings.HasPrefix(cleanHostPath, "//") {
		chapterHTML = strings.ReplaceAll(chapterHTML, cleanHostPath, scheme+cleanHostPath)
	}

	// 2. Generic catch-all for any src="//..." or href="//..."
	chapterHTML = strings.ReplaceAll(chapterHTML, `src="//`, fmt.Sprintf(`src="%s//`, scheme))
	chapterHTML = strings.ReplaceAll(chapterHTML, `src='//`, fmt.Sprintf(`src='%s//`, scheme))
	chapterHTML = strings.ReplaceAll(chapterHTML, `href="//`, fmt.Sprintf(`href="%s//`, scheme))
	chapterHTML = strings.ReplaceAll(chapterHTML, `href='//`, fmt.Sprintf(`href='%s//`, scheme))
